...
| `REDIS_URL` | API application | empty | When set, the user cache, the rate limiter, the JWT revocation blacklist, the idempotency store, and the background-job queue switch to Redis. Empty keeps everything in-process and per-instance. |
